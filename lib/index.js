...
internals.implementation = function (server, options) {
    ...
    var requestOptions = function(request) {
        var options = {};
        if (dynamicDomain) {
        ...
        request.auth.session = {
            set: function (session, value) {
                ...
                if (arguments.length > 1) {
                    ...
                    session[key] = value;
                    return reply.state(settings.cookie, session, requestOptions(request));
                }

                Hoek.assert(session && typeof session === 'object', 'Invalid session');
                request.auth.artifacts = session;
                reply.state(settings.cookie, session, requestOptions(request));
            },
            clear: function (key) {
                ...
                reply.unstate(settings.cookie, requestOptions(request));
            },
            ttl: function (msecs) {

                var session = request.auth.artifacts;
                var options = requestOptions(request);
                options.ttl = msecs;
